export default function CardPosts() {
  ...
  return (
    <div className="w-[440px] flex flex-col items-center">
      <img src={post.banner} alt={post.title} className="w-full h-[244px]" />
      <div>
        <p className="text-[clamp(0,5rem,4vw,0.875rem)] font-semibold">
          {post.title}
        </p>
        <div className="mt-2.5">
          ...
          <p className="text-[clamp(0,5rem,4vw,0.875rem)]">{post.about}</p>
        </div>
      </div>
    </div>
  );
